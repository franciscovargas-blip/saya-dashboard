...
export default function Dashboard(){
  ...
  const kpis=useMemo(()=>{
    ...
    const gpYTD=gS(fd,"Gross Profit","Forecast",ytdM),gpFY=gS(fd,"Gross Profit","Forecast",allM);
    const cogsYTD=gS(fd,"COGS","Forecast",ytdM),cogsFY=gS(fd,"COGS","Forecast",allM);
    const opexCM=gOx(fd,[cm]),opexYTD=gOx(fd,ytdM),opexFY=gOx(fd,allM);
    const gmYTD=nsYTD?gpYTD/nsYTD:0;
    const burnYTD=opexYTD-(nsYTD>0?nsYTD:0);
    const topOpex=OC.map(c=>({c,v:fd.filter(r=>r[0]===c&&r[1]==="Reales"&&r[2]===cm).reduce((s,r)=>s+r[8],0)})).filter(x=>x.v>0).sort((a,b)=>b.v-a.v);
    const topLabel=topOpex[0]?OX[topOpex[0].c]:"—";
    return [
      {l:`SALES ${MO[cm-1]}`,v:F(nsCM),s:`YTD ${F(nsYTD)}`,c:"#1D9E75",bc:"#D4F0E6"},
      {l:"NET SALES FY",v:F(nsFY),s:"Forecast anual",c:"#0B6644",bc:"#D4F0E6"},
      {l:`EBITDA ${MO[cm-1]}`,v:F(ebitdaCM),s:`YTD ${F(ebitdaYTD)}`,c:ebitdaCM<0?"#E24B4A":"#1D9E75",bc:ebitdaCM<0?"#FCEBEB":"#D4F0E6"},
      {l:"GROSS PROFIT YTD",v:F(gpYTD),s:nsYTD?`Margen ${P(gmYTD)}`:"Pre-revenue",c:"#185FA5",bc:"#E6F1FB"},
      {l:"COGS YTD",v:F(cogsYTD),s:`FY FC ${F(cogsFY)}`,c:"#BA7517",bc:"#FAEEDA"},
      {l:`OPEX ${MO[cm-1]}`,v:F(opexCM),s:`YTD ${F(opexYTD)}`,c:"#E24B4A",bc:"#FCEBEB"},
      {l:"BURN RATE YTD",v:F(burnYTD),s:`Top: ${topLabel}`,c:"#8B5CF6",bc:"#EEEDFE"},
      {l:"OPEX FY REAL",v:F(opexFY),s:`${OC.filter(c=>fd.some(r=>r[0]===c&&r[1]==="Reales")).length} categorías`,c:"#534AB7",bc:"#EEEDFE"},
    ];
  },[fd,cm,ytdM]);
  ...
  const rows=useMemo(()=>[...PLO,...OC].map(line=>{
    ...
    const ld=fd.filter(r=>r[0]===line);
    const monthly=[];for(let m=1;m<=12;m++){const fc=ld.filter(r=>r[2]===m&&r[1]==="Forecast").reduce((s,r)=>s+r[8],0);const re=ld.filter(r=>r[2]===m&&r[1]==="Reales").reduce((s,r)=>s+r[8],0);monthly.push(isO?re:(m<=cm&&re?re:fc))}
  ...
  const generateNarrative=useCallback(async()=>{
    ...
    const opexByCode=OC.map(c=>({name:OX[c],val:gOx(fd.filter(r=>r[0]===c),[cm])})).filter(x=>x.val!==0);
    const topPartners=pareto.slice(0,5).map(p=>`${p.partner}: ${F(p.total)}`);
    const summary=`Mes: ${MO[cm-1]} 2026. Net Sales FC: ${F(gS(fd,"Net Sales","Forecast",[cm]))}. EBITDA FC: ${F(gS(fd,"EBITDA","Forecast",[cm]))}. Gross Profit YTD: ${F(gS(fd,"Gross Profit","Forecast",ytdM))}. OpEx real mes: ${F(gOx(fd,[cm]))}. OpEx YTD: ${F(gOx(fd,ytdM))}. Desglose OpEx mes: ${opexByCode.map(x=>`${x.name}: ${F(x.val)}`).join(", ")}. Top partners mes: ${topPartners.join(", ")}. Moléculas con gasto: ${[...new Set(fd.filter(r=>r[1]==="Reales"&&r[2]===cm).map(r=>r[3]))].join(", ")}. COGS YTD: ${F(gS(fd,"COGS","Forecast",ytdM))}. First revenue month: Jun 2026.`;
    try{
      const resp=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,messages:[{role:"user",content:`Eres el CFO de Saya Biologics, una startup de biologics/pharma en México. Genera un análisis ejecutivo en español de máximo 4 párrafos basado en estos datos del rolling forecast. Sé específico con los números. Identifica drivers de gasto, riesgos, y oportunidades. Habla en primera persona plural. Datos: ${summary}`}]})});
      const data=await resp.json();
      const text=data.content?.map(c=>c.text||"").join("")||"No se pudo generar la narrativa.";
      setNarrative(text);
    }catch(e){setNarrative("Error al conectar con la API. Verifica tu conexión.")}
    setLoading(false);
  },[fd,cm,ytdM,pareto]);
  ...
  const btn={padding:"6px 14px",borderRadius:6,border:"none",fontSize:10,fontFamily:"inherit",cursor:"pointer",fontWeight:500};

  return(
    <div ref={ref} style={{fontFamily:"'DM Mono','Consolas',monospace",minHeight:"100vh"}}>
      <link href="https://fonts.googleapis.com/css2?family=DM+Mono:wght@400;500&family=Fraunces:ital,wght@0,300;0,500&display=swap" rel="stylesheet"/>
      <style>{`@media print{button,.no-print{display:none!important}}`}</style>

      {/* HEADER */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 0",borderBottom:"1px solid #E4E8F2",marginBottom:16,flexWrap:"wrap",gap:10}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <svg width="42" height="42" viewBox="0 0 42 42"><defs><linearGradient id="lg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="#1D9E75"/><stop offset="100%" stopColor="#085041"/></linearGradient></defs><rect width="42" height="42" rx="10" fill="url(#lg)"/><text x="21" y="17" textAnchor="middle" fill="#fff" fontSize="9" fontWeight="700" fontFamily="'Fraunces',serif" dominantBaseline="middle">SAYA</text><text x="21" y="29" textAnchor="middle" fill="#ffffff99" fontSize="7" fontWeight="400" fontFamily="'DM Mono',monospace" dominantBaseline="middle">BIOLOGICS</text></svg>
          <div>
            <div style={{fontFamily:"'Fraunces',serif",fontSize:20,fontWeight:500,color:"#1a1a2e",letterSpacing:-0.5}}>Financial Intelligence</div>
            <div style={{fontSize:9,color:"#8A90A8",letterSpacing:2,textTransform:"uppercase"}}>Rolling Forecast 2026 · Actualizado {MO[cm-1]}</div>
          </div>
        </div>
        <div className="no-print" style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <button onClick={exportPDF} style={{...btn,background:"#1a1a2e",color:"#fff"}}>📄 Exportar PDF</button>
          <button onClick={generateNarrative} disabled={loading} style={{...btn,background:"linear-gradient(135deg,#1D9E75,#085041)",color:"#fff",opacity:loading?.6:1}}>{loading?"⏳ Analizando...":"🤖 Narrativa IA"}</button>
        </div>
      </div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>
        ...
        <span style={{fontSize:8,padding:"3px 8px",borderRadius:4,background:"#EEEDFE",color:"#534AB7"}}>▶ Clic en filas para drill-down</span>
      </div>

      {/* AI NARRATIVE */}
      {narrative&&<div style={{background:"linear-gradient(135deg,#f0faf6,#E6F1FB)",border:"1px solid #1D9E7533",borderRadius:10,padding:"14px 18px",marginBottom:16,position:"relative"}}>
        <div style={{fontSize:10,fontWeight:500,color:"#085041",marginBottom:6,display:"flex",alignItems:"center",gap:6}}>🤖 Análisis Ejecutivo — {MO[cm-1]} 2026<button className="no-print" onClick={()=>setNarrative(null)} style={{marginLeft:"auto",background:"none",border:"none",cursor:"pointer",fontSize:14,color:"#8A90A8"}}>×</button></div>
        <div style={{fontSize:10,color:"#1a1a2e",lineHeight:1.6,whiteSpace:"pre-wrap"}}>{narrative}</div>
      </div>}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
        {[{t:`YTD ${MO[0]}–${MO[cm-1]}`,rk:"rY",fk:"fY",vk:"vY",pk:"pY"},{t:`Mes — ${MO[cm-1]}`,rk:"rC",fk:"fC",vk:"vC",pk:"pC"}].map((t,ti)=>(
          <div key={ti} style={{background:"#fff",border:"1px solid #E4E8F2",borderRadius:10,padding:12,overflowX:"auto"}}>
            ...
            <tbody>{comp.map((r,i)=><tr key={i} style={{background:BL.has(r.l)?"#fafbfe":"transparent"}}>
              ...
              <td style={{...td,textAlign:"right",color:vc(r[t.pk])}}>{P(r[t.pk])}</td>
            </tr>)}</tbody></table>
          </div>
        ))}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
        ...
        <div style={{background:"#fff",border:"1px solid #E4E8F2",borderRadius:10,padding:12}}>
          <div style={{fontSize:11,fontWeight:500,color:"#1a1a2e",marginBottom:2}}>Cascada P&L — YTD {MO[0]}–{MO[cm-1]}</div>
      ...
      <div style={{background:"#fff",border:"1px solid #E4E8F2",borderRadius:10,padding:12,marginBottom:16,overflowX:"auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:11,fontWeight:500,color:"#1a1a2e"}}>P&L Anual — Rolling Forecast</span></div>
      ...
      <div style={{textAlign:"center",fontSize:8,color:"#B0B6CC",padding:"12px 0",marginTop:8}}>Saya Biologics — Financial Intelligence · SAP + Excel → Rolling Forecast · 2026</div>
    </div>
  );
}
